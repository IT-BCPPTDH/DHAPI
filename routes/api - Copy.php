    ...
    foreach ($result as $row) {
        ...
        if (!isset($formattedData[$headerKey])) {
            $formattedData[$headerKey] = [
                ...
                "po_number" => $row->po_no,
                "po_date_time" => $row->creation_date,
        ...
        $formattedData[$headerKey]['items'][] = [
            "item_no" => $row->po_item_no,
            "stock_code" => $row->preq_stk_code,
            "PN" => $row->part_no,
            ...
            "unit_price" => $row->gross_price_p,
        ];
    }
    $formattedArray = array_values($formattedData);

    return response()->json(["data" => $formattedArray]);
...
Route::get("/GetWHSbyDsctStck",function(Request $request)
{
    ...
    $data =  DB::table('V_ITEMASTER_SCM as a')
...
Route::get("/getfinalco",function(Request $request)
{
    ...
    if ($maxId !== null) {
        ...
        $subquery = DB::table('v_itemaster_scm')
